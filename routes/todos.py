...
from schemas import TodoResponse, TodoSchema, TodoUpdateSchema
from repository import todos as repository_todos
...
async def update_todo(body: TodoUpdateSchema, todo_id: int = Path(ge=1), db: AsyncSession = Depends(get_db), user: User = Depends(auth_service.get_current_user)):
...
async def delete_todo(todo_id: int = Path(ge=1), db: AsyncSession = Depends(get_db), user: User = Depends(auth_service.get_current_user)):
